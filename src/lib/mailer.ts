const FROM_EMAIL = process.env.SMTP_USER; // 🔥 IMPORTANT FIX
...
export async function sendTemporaryPasswordEmail(
  ...
  if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
    console.warn(
      "SMTP not configured — skipping email send. Temporary password:",
      tempPassword
    );
    return false;
  }
  ...
  try {
    ...
    if (!mod) {
      console.warn(
        "nodemailer not installed — temporary password:",
        tempPassword
      );
      return false;
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      // 🔥 FIXED TLS CONFIG (safe for your broken enterprise SMTP)
      tls: {
    ...
    const loginUrl =
      process.env.LOGIN_URL || "https://nibaudit.nibbank.com.et";

    const html = getLoginCredentialsEmailTemplate(
      to,
      tempPassword,
      loginUrl
    );
    ...
    const info = await transporter.sendMail({
      from: `"NIB Audit System" <${SMTP_USER}>`, // 🔥 FIXED SENDER RULE
      to,
